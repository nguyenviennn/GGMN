<?php
    ...
    $sqll = "SELECT * FROM hoso WHERE Khu_Vuc = '$kv'";
    $tong = mysqli_query($conn, $sqll);
   $tongtk = mysqli_num_rows($tong);
   $sotrang = ceil($tongtk / $soluong);   
    if(isset($_POST["tukhoa"]) && $tukhoa != ""){
        $sql_thongke = "SELECT * FROM hoso WHERE Khu_Vuc = '$kv'AND Ho_Ten LIKE '%".$tukhoa."%'";
        $tk = mysqli_query($conn, $sql_thongke);
    }else{
        $sql_thongke = "SELECT * FROM hoso WHERE Khu_Vuc = '$kv' LIMIT $soluong OFFSET $vitritrongtrang "; 
        $tk = mysqli_query($conn, $sql_thongke);
    }
        ...
        <div class="container">
            ...
            <table class="table">
                <div class="chucnang">
                    ...
                    <form
                        action="../thongke/thongkekv.php?id=<?php echo $_GET['id'];?>&tukhoa=<?php echo $tukhoa ?>&kv=<?php echo $kv?>"
                        method="post">
                        <div class="search">
            ...
            <div class="nextpage">
                <ul class="nextpage">
                    <?php if($vitri >3){
                            $trangmot =1 ;?>
                    <li><a href="?id=<?=$id?>&soluong=<?=$soluong?>&vitri=<?=$trangmot?>&kv=<?=$kv?>">First</a></li>
                    <?php } ?>
                    <?php if($vitri > 1 ){
                            $quaylai = $vitri - 1 ;?>
                    <li><a href="?id=<?=$id?>&soluong=<?=$soluong?>&vitri=<?=$quaylai?>&kv=<?=$kv?>">Prev</a></li>
                    <?php } ?>
                    <?php for($num = 1 ;$num <= $sotrang;$num++){ ?>
                    <?php if($num != $vitri){ ?>
                    <?php if($num > $vitri - 3 && $num < $vitri +3){ ?>
                    <li><a href="?id=<?=$id?>&soluong=<?=$soluong?>&vitri=<?=$num?>&kv=<?=$kv?>"><?=$num?></a></li>
                    <?php }?>
                    <?php }else{ ?>
                    <strong><?=$num?></strong>
                    <?php }?>
                    ...
                    <?php if($vitri < $sotrang){
                            $tienlen = $vitri + 1 ;?>
                    <li><a href="?id=<?=$id?>&soluong=<?=$soluong?>&vitri=<?=$tienlen?>&kv=<?=$kv?>">Next</a></li>
                    <?php } ?>
                    <?php if($vitri < $sotrang - 2){
                            $trangcuoi = $sotrang ;?>
                    <li><a href="?id=<?=$id?>&soluong=<?=$soluong?>&vitri=<?=$trangcuoi?>&kv=<?=$kv?>">Last</a></li>
                    <?php } ?>
                </ul>
            </div>
        </div>
