<?php
    ...
    $sqll = "SELECT * FROM hoso WHERE Dot_Thi = '$dt'";
    $tong = mysqli_query($conn, $sqll);
   $tongtk = mysqli_num_rows($tong);
   $sotrang = ceil($tongtk / $soluong);   
    if(isset($_POST["tukhoa"]) && $tukhoa != ""){
        $sql_thongke = "SELECT * FROM hoso WHERE Dot_Thi = '$dt' AND Ho_Ten LIKE '%".$tukhoa."%' ";
        $tk = mysqli_query($conn, $sql_thongke);
    }else{
        $sql_thongke = "SELECT * FROM hoso WHERE Dot_Thi = '$dt' LIMIT $soluong OFFSET $vitritrongtrang ";
        $tk = mysqli_query($conn, $sql_thongke);
    }
        ...
        <div class="container">
            ...
            <table class="table">
                <div class="chucnang">
                    ...
                    <form action="../thongke/thongkedt1.php?id=<?php echo $_GET['id'];?>" method="post">
                        <div class="search">
                ...
                <thead>
                    <tr>
                        ...
                        <th scope="col">Năm dự thi</th>
                    </tr>
                </thead>
                <tbody>
                    <?php   
                        $i= 1;
                        while($rows = mysqli_fetch_array($tk)){ ?>
                    <tr>
            ...
            <div class="nextpage">
                <ul class="nextpage">
                    <?php if($vitri >3){
                            $trangmot =1 ;?>
                    <li><a href="?id=<?=$id?>&soluong=<?=$soluong?>&vitri=<?=$trangmot?>&dt=<?=$dt?>">First</a></li>
                    <?php } ?>
                    <?php if($vitri > 1 ){
                            $quaylai = $vitri - 1 ;?>
                    <li><a href="?id=<?=$id?>&soluong=<?=$soluong?>&vitri=<?=$quaylai?>&dt=<?=$dt?>">Prev</a></li>
                    <?php } ?>
                    <?php for($num = 1 ;$num <= $sotrang;$num++){ ?>
                    <?php if($num != $vitri){ ?>
                    <?php if($num > $vitri - 3 && $num < $vitri +3){ ?>
                    <li><a href="?id=<?=$id?>&soluong=<?=$soluong?>&vitri=<?=$num?>&dt=<?=$dt?>"><?=$num?></a></li>
                    <?php }?>
                    <?php }else{ ?>
                    <strong><?=$num?></strong>
                    <?php }?>
                    ...
                    <?php if($vitri < $sotrang){
                            $tienlen = $vitri + 1 ;?>
                    <li><a href="?id=<?=$id?>&soluong=<?=$soluong?>&vitri=<?=$tienlen?>&dt=<?=$dt?>">Next</a></li>
                    <?php } ?>
                    <?php if($vitri < $sotrang - 2){
                            $trangcuoi = $sotrang ;?>
                    <li><a href="?id=<?=$id?>&soluong=<?=$soluong?>&vitri=<?=$trangcuoi?>&dt=<?=$dt?>">Last</a></li>
                    <?php } ?>
                </ul>
            </div>
        </div>
