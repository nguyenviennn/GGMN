...
<body>
    <header>
        <form action="../quantri/quantritk.php" method="post">
        <div class="container">
            <div class="menu-top">
                ...
                <div class="search">
                    <input type="text" name="tukhoa" id="search" placeholder="Tìm kiếm tên tài khoản">
                ...
                if(isset($_POST["tukhoa"]) && $_POST["tukhoa"] != ""){
                    $sql = "SELECT * FROM login WHERE Tai_Khoan LIKE '%".$tukhoa."%'";
                    $rs = mysqli_query($conn, $sql);
                }else{
                    $sql = "SELECT * FROM login LIMIT $soluong OFFSET $vitritrongtrang";
                    $rs = mysqli_query($conn, $sql);
                }
                ...
                <div class="table">
                    <table>
                        <tr>
                            ...
                            <th>Tình trạng bài thi</th>
                            <th colspan="3">Quản lý</th>
                        </tr>
                        <tr>
                        <?php   
                        $i= 1;
                        while($rows = mysqli_fetch_array($rs)){ $idtong = $rows['Id']?>
                        <tr> 
                            ...
                            <td><?php if($rows['Duyet'] == "Sai thông tin cá nhân"){ echo '<div style="color:red;">Sai thông tin cá nhân</div>';}
                            if($rows['Duyet'] == "Thông tin chính xác"){ echo '<div style="color:green;">Đã xác thực</div>';}
                            if($rows['Duyet'] == "" && $rows['Loai_Tk'] != 1 && $rows['Loai_Tk'] != 3){ echo '<div style="color:blue;">Chưa xác thực</div>';}
                            if($rows['Loai_Tk'] == 1){ echo '<div style="color:black;">Quản trị viên</div>';}
                            if($rows['Loai_Tk'] == 3 ){ echo '<div style="color:black;">GV chấm thi</div>';}?></td>
                            <td><?php if($rows['baithi'] == 0){ echo '<div style="color:red;">Chưa nộp bài thi</div>';}
                            if($rows['baithi'] == 1 ){ echo '<div style="color:green;">Đã nộp bài thi</div>';}?></td>
                            ...
                            <td class="delete"><a href="../quantritk/xoatk.php?id=<?php echo $rows['Id']; ?>" onclick="return confirm('Bạn thực sự muốn xóa !!!')"> <?php if($rows['Tai_khoan'] != 'admin'){echo '<i class="fa-solid fa-xmark">';} ?></i></a></td>
                        </tr>
                        <?php } ?>
                    </table>
                </div>
