<?php
    ...
    while($row = $x->fetch_assoc()) { 
        if($id == $row['Id']){
            $thongbao = "Tài khoản đã gửi bài thi vui lòng kiểm tra lại!!!";
    ...
    $target_dir = "video/";
                ...
                $r = mysqli_fetch_array($x);
                  ...
                  $pt="SELECT * FROM phongthi WHERE Id_Phong = $phong";
                  $qr = mysqli_query($conn, $pt);
                  $b = mysqli_fetch_array($qr);
                    if($r['Dot_Thi'] != $b['Doi_Thi']){
                       $_SESSION['phong'] = "Tài khoản của bạn không ở trong đợt thi này !!!";
                       header("Location: ../phongthi/phongthi.php");
                    }else{
    ...
    <section class="container">
        <div class="form-dangki">
            ...
            <form action="baithi1.php?id=<?php echo $_SESSION['id'] ?>" method="post" enctype="multipart/form-data"
                ...
                if(isset($loi)&&$loi != ""){ ?>
                    <div style="color:green;"><?php echo $loi; ?></div>
                    <?php }?>
                <?php
                    if($thongbao != ""){ ?>
                ...
                <div class="row">
                    <div class="col">
                        <div class="form-group">
                            <label for="exampleFormControlFile1">File bài thi (Bài thi có dạng .mp4): </label>
        ...
        <!-- <div>
                        <a href="https://pwa.zoom.us/wc/">Thi qua phầm mềm zoom </a>
        </div> -->
        </div>
    </section>
    <footer class="footer">
